...
class masterTestCase(APITestCase):
    ...
    def setUp(self):
        ...
        self.client.post('/master/component_list/', data=data[0], format='json')
        self.client.post('/master/component_list/', data=data[1], format='json')
        self.client.post('/master/component_list/', data=data[2], format='json')

    def test_get_big_data_components_list(self):
        response = self.client.get('/master/component_list/')
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(len(response.data[0].keys()), 3)

    def test_get_first_big_data_component(self):
        response = self.client.get('/master/component_list/1/')
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        ...
        response = self.client.get('/master/component_list/2/')
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        ...
        response = self.client.get('/master/component_list/3/')
        self.assertEqual(response.status_code, status.HTTP_200_OK)
